...
class UserController extends BaseController
{
    ...
    public function getLoginUrlAction()
    {
        ...
        if (isWechat()) {
            return jsonResponse(['type' => 'go', 'alert' => getRequestQuery('alert') ? getRequestQuery('alert') : 'alert', 'url' => (new Jeemu\Wechat\UserInfo(conf('wechat.appid'), conf('wechat.appsecret')))->getBaseUrl(Jeemu\Dispatcher::getInstance()->getRequest()->host . '/api/wechat/code' . $path)], 1, '前去登录?');
        }
        return jsonResponse(['type' => 'push', 'alert' => getRequestQuery('alert') ? getRequestQuery('alert') : 'alert', 'url' => '/login' . $path], 1, '前去登录?');
    }
